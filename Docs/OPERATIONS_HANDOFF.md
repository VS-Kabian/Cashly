...
2. Review `supabase/migrations/20260714000000_harden_admin_authorization.sql` and `supabase/migrations/20260714000001_strengthen_financial_integrity.sql` in a staging project first.
3. Before applying the financial migration, resolve any duplicate overall budgets for the same `(user_id, month, year)`. The migration intentionally stops instead of deleting or choosing financial records.
...
1. Apply migrations to staging and run the database validation checklist in `Docs/DATABASE_VALIDATION.md`.
2. Smoke-test normal-user registration/sign-in, category creation, transaction create/update/delete, budget creation, history pagination/filtering, and the admin login flow.
3. Deliberately use an invalid amount, an empty category, and an invalid/empty date to confirm each inline validation message is announced and no transaction is saved.
4. In a staging browser session, force a rendering error through the browser tools and confirm the generic recovery screen appears without exception details; select **Try again** to recover.
5. Deploy the frontend only after staging succeeds, then apply the same migration sequence to production during a low-traffic window.
6. Repeat the smoke tests against production with a non-privileged account first, followed by an admin account.
